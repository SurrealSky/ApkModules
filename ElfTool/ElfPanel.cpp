...
IMPLEMENT_DYNAMIC(ElfPanel, CDialogEx)

ElfPanel::ElfPanel(CWnd* pParent /*=NULL*/)
	: CDialogEx(ElfPanel::IDD, pParent)
	, strFile(_T(""))
	, m_ident(_T(""))
	, m_type(_T(""))
	, m_machine(_T(""))
...
void ElfPanel::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Text(pDX, IDC_MFCEDITBROWSE1, strFile);
	DDX_Control(pDX, IDC_TREE1, m_Tree);
	DDX_Control(pDX, IDC_LIST1, m_List);
	DDX_Text(pDX, IDC_EDIT1, m_ident);
	DDX_Text(pDX, IDC_EDIT2, m_type);
	DDX_Text(pDX, IDC_EDIT3, m_machine);
	DDX_Text(pDX, IDC_EDIT4, m_version);
	DDX_Text(pDX, IDC_EDIT5, m_entry);
	DDX_Text(pDX, IDC_EDIT6, m_phoff);
	DDX_Text(pDX, IDC_EDIT7, m_shoff);
	DDX_Text(pDX, IDC_EDIT8, m_flags);
	DDX_Text(pDX, IDC_EDIT9, m_ehsize);
	DDX_Text(pDX, IDC_EDIT10, m_phentsize);
	DDX_Text(pDX, IDC_EDIT11, m_phnum);
	DDX_Text(pDX, IDC_EDIT12, m_shentsize);
	DDX_Text(pDX, IDC_EDIT13, m_shnum);
	DDX_Text(pDX, IDC_EDIT14, m_shstrndx);
	DDX_Text(pDX, IDC_EDIT15, m_total);
}


BEGIN_MESSAGE_MAP(ElfPanel, CDialogEx)
	ON_BN_CLICKED(IDC_BUTTON1, &ElfPanel::OnBnClickedButton1)
	ON_NOTIFY(NM_CLICK, IDC_TREE1, &ElfPanel::OnNMClickTree)
	ON_NOTIFY(NM_DBLCLK, IDC_TREE1, &ElfPanel::OnNMDblclkTree)
END_MESSAGE_MAP()
...
//分析
void ElfPanel::OnBnClickedButton1()
{
	UpdateData(TRUE);

	//需要等待线程等待完成
	if (mElfAnalyse.isAnalysised)
	{
		mElfAnalyse.elfUnload();
	}
	...
	//加载文件
	if (!mElfAnalyse.elfLoadFile(strFile.GetBuffer(0), L"r"))     //只读方式打开文件
	{
		::MessageBox(this->m_hWnd, L"加载elf文件失败", L"警告", MB_OK);
		return;
	}

	//开始分析
	if (mElfAnalyse.Analysis())
	{
		mElfAnalyse.isAnalysised = true;
		ToHead();
		ToTree();

		std::wstring str = mElfAnalyse.doReport();
		//strReport = L"";
...
BOOL ElfPanel::OnInitDialog()
{
	CDialogEx::OnInitDialog();

	// TODO:  在此添加额外的初始化
	strFile = L"C:\\Users\\Administrator\\Desktop\\libimagesdk.so";
	UpdateData(FALSE);
	return TRUE;  // return TRUE unless you set the focus to a control
	// 异常:  OCX 属性页应返回 FALSE
}

void ElfPanel::ToTree()
{
	if (mElfAnalyse.mCtx.elf.is64Bit)
	{
		std::map<CString, Elf64_Phdr>::iterator itor1;
		for (itor1 = mElfAnalyse.mCtx.elf.Map_Phdr64.begin(); itor1 != mElfAnalyse.mCtx.elf.Map_Phdr64.end(); itor1++)
		{
			m_Tree.InsertItem(itor1->first, 0, 2);
		}
		std::map<CString, Elf64_Shdr>::iterator itor2;
		for (itor2 = mElfAnalyse.mCtx.elf.Map_Shdr64.begin(); itor2 != mElfAnalyse.mCtx.elf.Map_Shdr64.end(); itor2++)
		{
			HTREEITEM Handle = m_Tree.InsertItem(itor2->first, 0, 2);
			if (itor2->first == L".shstrtab" || itor2->first == L".strtab" || itor2->first == L".dynstr")
			{
				m_Tree.InsertItem(L"String Table 字符串表", 1, 3, Handle);
			}
			else if (itor2->first == L".symtab")
			{
				m_Tree.InsertItem(L"Symtab 符号表", 1, 3, Handle);
			}
			else if (itor2->first == L".rel.dyn" || itor2->first == L".rel.plt")
			{
				m_Tree.InsertItem(L"REL Table 重定位表", 1, 3, Handle);
			}
			else if (itor2->first == L".dynsym")
			{
	...
	{
		std::map<CString, Elf32_Phdr>::iterator itor1;
		for (itor1 = mElfAnalyse.mCtx.elf.Map_Phdr32.begin(); itor1 != mElfAnalyse.mCtx.elf.Map_Phdr32.end(); itor1++)
		{
			m_Tree.InsertItem(itor1->first, 0, 2);
		}
		std::map<CString, Elf32_Shdr>::iterator itor2;
		for (itor2 = mElfAnalyse.mCtx.elf.Map_Shdr32.begin(); itor2 != mElfAnalyse.mCtx.elf.Map_Shdr32.end(); itor2++)
		{
			HTREEITEM Handle = m_Tree.InsertItem(itor2->first, 0, 2);
			if (itor2->first == L".shstrtab" || itor2->first == L".strtab" || itor2->first == L".dynstr")
			{
...
void ElfPanel::ToHead()
{
	if (mElfAnalyse.mCtx.elf.is64Bit)
	{
		Elf64_Ehdr* Elf_Ehdr = (Elf64_Ehdr*)(mElfAnalyse.mCtx.pVirMem);
		m_ident = L".ELF";
	...
	{
		Elf32_Ehdr* Elf_Ehdr = (Elf32_Ehdr*)(mElfAnalyse.mCtx.pVirMem);
		m_ident = L".ELF";
...
WCHAR*  ElfPanel::CharToWchar(char* szSour)
{
	WCHAR* wzBuffer = NULL;
	if (szSour)
	{
...
void ElfPanel::OnNMClickTree(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO:  在此添加控件通知处理程序代码
	*pResult = 0;
	...
	if (mElfAnalyse.mCtx.elf.is64Bit)
	{
		...
		{
			std::map<CString, Elf64_Phdr>::iterator it;
			it = mElfAnalyse.mCtx.elf.Map_Phdr64.find(pPath);
		...
		else if (pPath == L"String Table 字符串表")
		{
			...
			std::map<CString, Elf64_Shdr>::iterator it;
			it = mElfAnalyse.mCtx.elf.Map_Shdr64.find(pParentPath);
		...
		else if (pPath == L"Dynsym 符号表" || pPath == L"Symtab 符号表")
		{
			...
			{
				it = mElfAnalyse.mCtx.elf.Map_Shdr64.find(L".strtab");
				DynString = it->second;
			}
			else if (pPath == L"Dynsym 符号表")
			{
				it = mElfAnalyse.mCtx.elf.Map_Shdr64.find(L".dynstr");
				DynString = it->second;
			}
		...
		{
			std::map<CString, Elf64_Shdr>::iterator it;
			it = mElfAnalyse.mCtx.elf.Map_Shdr64.find(pPath);
			Elf64_Shdr Temp = it->second;
	...
	else
	{
		if (pPath.Find(L"Program Header") == 0)
		{
			std::map<CString, Elf32_Phdr>::iterator it;
			it = mElfAnalyse.mCtx.elf.Map_Phdr32.find(pPath);
		...
		else if (pPath == L"String Table 字符串表")
		{
			...
			std::map<CString, Elf32_Shdr>::iterator it;
			it = mElfAnalyse.mCtx.elf.Map_Shdr32.find(pParentPath);
		...
		else if (pPath == L"Dynsym 符号表" || pPath == L"Symtab 符号表")
		{
			...
			if (pPath == L"Symtab 符号表")
			{
				it = mElfAnalyse.mCtx.elf.Map_Shdr32.find(L".strtab");
				DynString = it->second;
			}
			else if (pPath == L"Dynsym 符号表")
			{
				it = mElfAnalyse.mCtx.elf.Map_Shdr32.find(L".dynstr");
				DynString = it->second;
			}
		...
		{
			std::map<CString, Elf32_Shdr>::iterator it;
			it = mElfAnalyse.mCtx.elf.Map_Shdr32.find(pPath);
			Elf32_Shdr Temp = it->second;
...
void ElfPanel::OnNMDblclkTree(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO:  在此添加控件通知处理程序代码
	*pResult = 0;
	HTREEITEM hCurItem = m_Tree.GetSelectedItem();
...
VOID ElfPanel::InitListControl(int number, COLUMNSTRUCT* Item)
{
...
VOID ElfPanel::InsertPhdr32(Elf32_Phdr Phdr)
{
...
VOID ElfPanel::InsertPhdr64(Elf64_Phdr Phdr)
{
	CString p_type, p_offset, p_vaddr, p_paddr, p_filesz, p_memsz, p_flags, p_align;
	p_type.Format(L"%d", Phdr.p_type);
	p_offset.Format(L"0x%x", Phdr.p_offset);
...
VOID ElfPanel::InsertString32(Elf32_Shdr Shdr)
{
	char* szString = mElfAnalyse.mCtx.pVirMem + Shdr.sh_offset;
	int i = 0;
...
VOID ElfPanel::InsertString64(Elf64_Shdr Shdr)
{
	char* szString = mElfAnalyse.mCtx.pVirMem + Shdr.sh_offset;  //越过内存中 一个""
	int i = 0;
	char* oriAddr = szString;
	while (TRUE)
	{
...
VOID ElfPanel::InsertSym32(Elf32_Shdr Shdr, Elf32_Shdr DynString)
{
	Elf32_Sym* Sym = (Elf32_Sym*)(mElfAnalyse.mCtx.pVirMem + Shdr.sh_offset);

	for (int i = 0; i<Shdr.sh_size / sizeof(Elf32_Sym); i++)
	{
		CString st_name, st_value, st_size, st_info1, st_other, st_shndx;

		char* szName = mElfAnalyse.mCtx.pVirMem + DynString.sh_offset + Sym->st_name;
...
VOID ElfPanel::InsertSym64(Elf64_Shdr Shdr, Elf64_Shdr DynString)
{
	Elf64_Sym* Sym = (Elf64_Sym*)(mElfAnalyse.mCtx.pVirMem + Shdr.sh_offset);
...
VOID ElfPanel::InsertShdr32(Elf32_Shdr Shdr)
{
...
VOID ElfPanel::InsertShdr64(Elf64_Shdr Shdr)
{
	CString sh_name, sh_type, sh_flags, sh_addr, sh_offset, sh_size, sh_link, sh_info, sh_addralign, sh_entsize;
	sh_name.Format(L"%d", Shdr.sh_name);
	sh_type.Format(L"%d", Shdr.sh_type);
...
VOID ElfPanel::InsertRel32(Elf32_Shdr Shdr)
{
	if (!mElfAnalyse.mCtx.elf.isNoSPARC)
	{
		Elf32_Rel* Rel = (Elf32_Rel*)(mElfAnalyse.mCtx.pVirMem + Shdr.sh_offset);
...
//!!
VOID ElfPanel::InsertRel64(Elf64_Shdr Shdr)
{

	Elf64_Rela* Rel = (Elf64_Rela*)(mElfAnalyse.mCtx.pVirMem + Shdr.sh_offset);
